...
plotFile  = 'Aftershock_plot.png' #file to save plot
...
dPar  =  { 'k'    : 5, # TODO: change this parameter to see how fit and R2 will changes
           ...
           'tmin'  : 1, 'tmax' : 100,
           'testPlot' : False, # set to False after initial debugging
           }
...
at_days = mData[0]*365.25 + mData[1]*(365.25/12) + mData[2]  + mData[3]/24 + mData[4]/(60*24) + mData[5]/(60*24*60)
...
dPL = opt_utils.lin_LS( np.log(at_bin_AS.T[sel_t].T) , np.log(aN_bin_AS.T[sel_t].T))
p_omori = dPL['b']
...
ax.loglog( at_bin_AS, aN_bin_AS , 'ko', mfc = 'none', mew = 1.5, label = 'aftershocks, $N_{tot}$=%i'%( at_bin_AS.shape[0]))
# :TODO plot modeled rates based on Omori law
ax.loglog( at_bin_AS, aOmori_rate , 'bo', mfc = 'none', mew = 1.5, label = 'aftershocks, $N_{tot}$=%i'%( at_bin_AS.shape[0]))
